...
const RELEASE_NOTES_DIR = `${APP_DIR}/release-notes`;
...
const replaceBrackets = str => str
  .replace(/\{\{/g, '<span ngNonBindable>{{</span>')
  .replace(/\}\}/g, '<span ngNonBindable>}}</span>')
  .replace(/\{/g, '&#123;')
  .replace(/\}/g, '&#125;');

const markdownToHtml = markdown => replaceBrackets(marked(markdown, {
...
const upperCamelCase = str => _.words(str).map(word => _.upperFirst(word)).join('');
...
const saveDocRoutingModule = (moduleName, data) => writeFile(`${APP_DIR}/${moduleName}/${moduleName}-routing.module.ts`, data);
...
const createDocs = (file, pathDir, moduleName) => {
  ...
  doc.html = compiledTemplate(DOC_VIEW_TEMPLATE, {label: doc.label, html: doc.html});
  saveDocView(pathDir, doc.fileName, moduleName, doc.html);

  doc.component = compiledTemplate(DOC_COMPONENT_TEMPLATE, {component: doc.name, templateUrl: doc.fileName});
...
const createDocsModule = (docs, moduleName) => {
  ...
  docs.forEach(doc => menuItems[moduleName].push({
    label: doc.label,
    link: doc.link,
    name: doc.name,
    path: doc.fileName,
  ...
  const docModule = compiledTemplate(DOC_MODULE_TEMPLATE, {docItems: menuItems[moduleName]});
  saveDocModule(moduleName, docModule);

  const docRoutingModule =
    compiledTemplate(DOC_ROUTING_MODULE_TEMPLATE, {docItems: menuItems[moduleName]});
...
gulp.task('build', ['clean-docs'], () => gulp.start(['build:api', 'build:doc', 'build:analytics']));
gulp.task('build:doc', ['build:guides', 'build:release-notes'], () => gulp.start('build:menus'));
...
gulp.task('build:guides', () => {
  const util = require('./docs/util');
  const thfProjectsNames = util.projectsNames();
  let guides = [];
  let sources = [];

  mkdir(GUIDE_DIR);

  thfProjectsNames.forEach(project =>
...
gulp.task('build:release-notes', () => {
  ...
  thfProjectsNames.forEach(project =>
    sources.push(`${configuration.sourceFolder}/${project}/docs/release-notes/*.md`));

  return gulp.src(sources, {})
    .pipe(tap(file => releases.push(createDocs(file, RELEASE_NOTES_DIR, 'release-notes'))))
    .pipe(gulp.dest(file => file.base))
    .on('end', () => createDocsModule(releases, 'release-notes'));
});
 ...
 * @task {build:menus}
 */
gulp.task('build:menus', () => {
  const menuService = compiledTemplate(MENU_SERVICE_TEMPLATE, {menuItems});
  const menuGuidesService = compiledTemplate(MENU_GUIDES_SERVICE_TEMPLATE, {menuItems});
...
gulp.task('clean-docs', function (cb) {
  return del([
    ...
    'src/app/documentation/documentation-routing.module.ts'], cb)
});
...
gulp.task('sonarqube', function (callback) {
  ...
  sonarqubeScanner({
    serverUrl: "http://sonarqube.thf.totvs.com.br",
    token: token,
    options: { // Documentation: https://docs.sonarqube.org/display/SONAR/Analysis+Parameters
      "sonar.projectKey": "thf-portal",
      "sonar.projectName": "thf-portal",
      "sonar.projectVersion": "1.0",
      "sonar.exclusions": "index.ts,app.*.ts,components/components.module.ts",
